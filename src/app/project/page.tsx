...
const ProjectCard = ({ project, reverse }: { project: Project; reverse?: boolean }) => {
  ...
  return (
    <div
      ...
      } gap-8 md:gap-12 items-start mb-16 md:mb-24`}
    >
      {/* Image grid */}
      <div className="w-full md:w-1/2 flex flex-col sm:flex-row gap-3 sm:gap-4">
        {imageUrl.length === 3 ? (
          <>
            {/* Special 3-image layout */}
            <div className="sm:w-1/2 rounded-2xl overflow-hidden">
              <Image
                src={imageUrl[0]}
                alt={title}
                width={400}
            ...
            <div className="sm:w-1/2 flex flex-col gap-3 sm:gap-4">
              <div className="rounded-2xl overflow-hidden h-[120px] sm:h-[160px] md:h-[220px]">
                <Image
                  src={imageUrl[1]}
                  alt={title}
                  width={400}
          ...
          <>
            ...
            <div className="flex flex-col gap-2 w-full sm:w-1/3">
              {imageUrl
                .filter((_, i) => i % 2 === 0)
                .map((img, idx) => (
                  <div key={idx} className="rounded-2xl overflow-hidden h-[180px] sm:h-[220px] md:h-[250px]">
                    <Image
                      src={img}
      ...
      {/* Text content */}
      <div className="w-full md:w-1/2 py-6 md:py-20 text-center md:text-left">
        <h2 className="text-[#2c2c2c] text-2xl sm:text-3xl md:text-[42px] font-semibold leading-tight mb-4 md:mb-6">
          {title}
        </h2>
        <p className="text-[#666] text-sm sm:text-base md:text-[16px] leading-relaxed mb-4 md:mb-6">
          {description.slice(0, 200)}...
        </p>
        <div className="flex justify-center md:justify-start items-center gap-2 mb-6 md:mb-8">
          <span className="text-red-500 text-lg sm:text-xl">📍</span>
          <span className="text-[#2c2c2c] text-sm sm:text-base md:text-[16px] font-medium">
            {location}
          </span>
        </div>
        <Link href={`/all-project/${project.slug}`}>
          <button className="text-[#996830] text-sm sm:text-base font-semibold hover:underline flex items-center justify-center md:justify-start gap-2">
            View Project <span>→</span>
          </button>
        </Link>
      </div>
...
const ProjectPage = () => {
  ...
  return (
    <div className="bg-[#fffefa] w-full min-h-screen relative">
      {/* Hero Video */}
      <section className="relative h-[250px] sm:h-[400px] md:h-[600px]">
        <video
          ref={videoRef}
          className="w-full h-full object-cover"
          src="/Images/project-video.mp4"
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
          <button
            className="w-[80px] sm:w-[120px] md:w-[159px] h-[80px] sm:h-[120px] md:h-[159px] bg-white/30 rounded-full flex items-center justify-center hover:bg-white/50 transition text-2xl sm:text-3xl md:text-4xl"
            onClick={toggleVideo}
          >
            {isPlaying ? "⏸️" : "▶️"}
          </button>
      ...
      {/* Projects Section */}
      <section className="max-w-[1400px] mx-auto px-4 sm:px-8 md:px-[88px] py-12 sm:py-20 md:py-24">
        <div className="text-center mb-12 sm:mb-16 md:mb-20">
          <h1 className="text-[#2c2c2c] text-3xl sm:text-4xl md:text-[64px] font-bold leading-tight mb-3 md:mb-4">
            Our Featured Projects
          </h1>
          <p className="text-[#666] text-sm sm:text-base md:text-[20px] leading-relaxed max-w-[800px] mx-auto">
            Explore our portfolio of thoughtfully designed spaces that blend aesthetics with functionality.
          </p>
        </div>
        ...
        {projects.length > 0 ? (
          ...
          <div className="text-center text-gray-500 text-sm sm:text-base">Loading projects...</div>
        )}
      </section>
    </div>
  );
